...
def update_vocab_related_parameters(parser, loaded_vocab):
    ...
    copy_embedding_to_current_layer(pre_trained_src_vocab, parser.src_vocab, pre_trained_src_embedding,
                                                     parser.src_embed)


    pre_trained_tgt_vocab = parser.tgt_vocab
    parser.tgt_vocab = loaded_vocab.code

    pre_trained_tgt_emb = parser.tgt_emb
    parser.tgt_emb = nn.Embedding(len(parser.tgt_vocab), parser.embed_size)
    nn.init.xavier_normal_(parser.tgt_emb.weight.data)

    print("update tgt embedding")
    copy_embedding_to_current_layer(pre_trained_tgt_vocab, parser.tgt_vocab, pre_trained_tgt_emb,
                                                     parser.tgt_emb)



    pre_trained_readout_embedding = parser.readout
    parser.readout = nn.Linear(parser.hidden_size, len(parser.tgt_vocab), bias=False)
    nn.init.xavier_normal_(parser.readout.weight.data)


    print("update tgt readout")
    copy_embedding_to_current_layer(pre_trained_tgt_vocab, parser.tgt_vocab, pre_trained_readout_embedding,
                                    parser.readout)


    print (len(parser.tgt_vocab))
    parser.label_smoothing_layer = nn_utils.LabelSmoothing(parser.label_smoothing, len(parser.tgt_vocab), ignore_indices=[0])
...
class Seq2SeqModel(nn.Module):
    ...
    def forward(self, examples):
        ...
        """
        batch = Batch(examples, self.vocab, use_cuda=self.use_cuda, data_type='fewshot')
    ...
    def load(cls, model_path, use_cuda=False, loaded_vocab=None, args = None):
        ...
        if loaded_vocab:
            update_vocab_related_parameters(parser, loaded_vocab)
