...
@Registrable.register('seq2seq_complete')
class Seq2SeqModel(nn.Module):
    ...
    def __init__(self, vocab, args):
        ...
        self.add_module('src_embed', self.src_embed)

        self.ast_embed = nn.Embedding(len(self.vertex_vocab), self.action_embed_size)

        self.add_module('ast_embed', self.ast_embed)
        ...
        self.add_module('encoder_lstm', self.encoder_lstm)

        self.decoder_lstm = nn.LSTMCell(self.decoder_size, self.hidden_size)

        self.add_module('decoder_lstm', self.decoder_lstm)
        ...
        if self.use_input_lstm_encode:
            ...
            self.add_module('input_encode_lstm', self.input_encode_lstm)

        # initialize the decoder's state and cells with encoder hidden states
        self.decoder_cell_init = nn.Linear(self.hidden_size * 2, self.hidden_size)

        self.add_module('decoder_cell_init', self.decoder_cell_init)

        # attention: dot product attention
        # project source encoding to decoder rnn's h space
        self.att_src_linear = nn.Linear(self.hidden_size * 2, self.hidden_size, bias=False)

        self.add_module('att_src_linear', self.att_src_linear)

        # transformation of decoder hidden states and context vectors before reading out target words
        # this produces the `attentional vector` in (Luong et al., 2015)
        if self.args.align_att:
            #print ("linear")
            self.att_vec_linear = nn.Linear(self.hidden_size * 2 + self.hidden_size, self.hidden_size, bias=False)
            self.domain_att_vec_linear = nn.Linear(self.hidden_size * 4 + self.hidden_size, self.hidden_size, bias=False)
        else:
            self.att_vec_linear = nn.Linear(self.hidden_size * 2 + self.hidden_size, self.hidden_size, bias=False)

        self.add_module('att_vec_linear', self.att_vec_linear)
        ...
        self.add_module('input_stack_reduce_linear', self.input_stack_reduce_linear)

        self.readout_action = nn.Linear(self.hidden_size, len(self.action_vocab), bias=False)

        self.add_module('readout_action', self.readout_action)

        # entity prediction
        self.entity_lstm = nn.LSTMCell(self.decoder_size, self.hidden_size)

        self.add_module('entity_lstm', self.entity_lstm)
        ...
        self.add_module('readout_entity', self.readout_entity)

        # variable prediction
        self.variable_lstm = nn.LSTMCell(self.hidden_size, self.hidden_size)

        self.add_module('variable_lstm', self.variable_lstm)

        self.variable_embed = nn.Embedding(len(self.variable_vocab), self.hidden_size)

        self.add_module('variable_embed', self.variable_embed)
        ...
        self.add_module('readout_variable', self.readout_variable)
        ...
        if self.args.discriminate_loss:
            self.w_a_e = nn.Parameter(self.new_tensor(1, 2*self.hidden_size))
            self.discriminator_linear = nn.Linear(self.hidden_size*2, 1)

    def dot_prod_attention(self, h_t, src_encoding, src_encoding_att_linear, mask=None, domain_idx=None):
        """
        ...
        if self.args.align_att and domain_idx is not None:
            #print(domain_idx)
            #print(att_weight)
            att_weight = att_weight*domain_idx
            #print (att_weight)
    ...
    def encode(self, src_sents_var, src_sents_len, weight=None):
        """
        ...
        if weight is None:
            ...
            # print(weight.keys())
            src_token_embed = F.embedding(src_sents_var, weight['src_embed.weight'])
            if self.dropout_i:
                src_token_embed = self.dropout_i(src_token_embed)
            packed_src_token_embed = pack_padded_sequence(src_token_embed, src_sents_len)
            # src_encodings: (tgt_query_len, batch_size, hidden_size)

            input, batch_sizes, sorted_indices, unsorted_indices = packed_src_token_embed

            max_batch_size = batch_sizes[0]
            max_batch_size = int(max_batch_size)

            num_directions = 2 if self.encoder_lstm.bidirectional else 1
            zeros = torch.zeros(self.encoder_lstm.num_layers * num_directions,
                                max_batch_size, self.encoder_lstm.hidden_size,
                                dtype=input.dtype, device=input.device)
            hx = (zeros, zeros)

            # print (self.encoder_lstm._flat_weights_names)
            ...
            for layer in range(1):
                for direction in range(num_directions):
                    ...
                    param_names = ['encoder_lstm.weight_ih_l{}{}', 'encoder_lstm.weight_hh_l{}{}']

                    param_names += ['encoder_lstm.bias_ih_l{}{}', 'encoder_lstm.bias_hh_l{}{}']
            ...
            _flat_weights = [(lambda wn: weight[wn])(wn) for wn in _flat_weights_names]


            result =  _VF.lstm(input, batch_sizes, hx, _flat_weights, self.encoder_lstm.bias, self.encoder_lstm.num_layers,
                              self.encoder_lstm.dropout, self.encoder_lstm.training, self.encoder_lstm.bidirectional) # self.encoder_lstm(packed_src_token_embed)
            ...
            src_encodings, (last_state, last_cell) = output_packed, self.encoder_lstm.permute_hidden(hidden, unsorted_indices)
    ...
    def init_decoder_state(self, enc_last_state, enc_last_cell, weight=None):
        """Compute the initial decoder hidden state and cell state"""
        if weight is None:
            h_0 = self.decoder_cell_init(enc_last_cell)
            h_0 = torch.tanh(h_0)
        else:
            h_0 = F.linear(enc_last_cell, weight['decoder_cell_init.weight'], weight['decoder_cell_init.bias']) # self.decoder_cell_init(enc_last_cell)
            h_0 = torch.tanh(h_0)

        return h_0, self.new_tensor(h_0.size()).zero_()

    def decode(self, src_encodings, src_sent_masks, dec_init_vec, batch, weight=None):
        """
        ...
        if weight is None:
            ...
            src_encodings_att_linear = F.linear(src_encodings, weight['att_src_linear.weight'])
        ...
        if weight is None:
            ...
            ast_seq_embed = F.embedding(ast_seq_var, weight['ast_embed.weight'])
            if len(batch.variable_seq[0]) > 0:
                var_seq_embed = F.embedding(batch.variable_seq_var, weight['variable_embed.weight'])
        ...
        if self.args.align_att:
            ...
            qc = self.new_tensor(len(batch), src_encodings.size(1)).fill_(self.args.align_att)
            #print (action_seq)
            for id in action_seq[0][-1].align_ids:
                qc[:,id] = 1
        ...
        for t, y_tm1_embed in list(enumerate(ast_seq_embed.split(split_size=1)))[:-1]:
            ...
            if isinstance(current_node, ReduceAction):
                ...
                if weight is None:
                    ...
                    x_point = F.linear(torch.cat(
                        [children_embedding, x], 1), weight['input_stack_reduce_linear.weight'])
            ...
            if len(current_node.entities) > 0:
                #print (current_node)
                assert len(current_node.entities) == 1, "one action only has one entity"
                score_e_t = self.readout_entity(att_tm1)
            ...
            if len(current_node.entities) > 0:
                ...
                for ent in current_node.entities:
                    ...
                    (h_e, cell_e), att_t_e, att_weight_e = self.step(x, h_tm1_e, src_encodings,
                                                                     ...
                                                                     src_sent_masks=src_sent_masks, lstm_type='entity',weight=weight)
                    if weight is None:
                        score_e_t = self.readout_entity(att_t_e)
                    else:
                        score_e_t = F.linear(att_t_e, weight['readout_entity.weight'])
            ...
            if len(current_node.variables) > 0:
                ...
                for var in current_node.variables:
                    ...
                    (h_t, cell_t), att_t, att_weight = self.step(input_v, hyp.v_hidden_embedding,
                                                                 ...
                                                                 lstm_type='variable',weight=weight)
                    if weight is None:
                        score_v_t = self.readout_variable(att_t)
                    else:
                        score_v_t = F.linear(att_t, weight['readout_variable.weight'])
            ...
            (h_t, cell_t), att_t, att_weight = self.step(x_point, h_tm1_point, src_encodings,
                                                         src_encodings_att_linear_cov,
                                                         src_sent_masks=src_sent_masks,weight=weight, domain_idx = qc)
            # (batch_size, tgt_vocab_size)
            if weight is None:
                score_t = self.readout_action(att_t)  # E.q. (6)
            else:
                score_t = F.linear(att_t, weight['readout_action.weight'])
            # print (re_score_t)
    ...
    def step(self, x, h_tm1, src_encodings, src_encodings_att_linear, src_sent_masks=None, lstm_type='decoder', weight = None, domain_idx=None):
        """
        ...
        if weight is None:
            ...
            #print (domain_idx)
            if self.args.align_att and domain_idx is not None:
                domain_ctx_t, domain_alpha_t = self.dot_prod_attention(h_t, src_encodings, src_encodings_att_linear,
                                                         mask=src_sent_masks, domain_idx=domain_idx)
                #print(torch.cat([h_t, ctx_t], 1).size())
                #print (torch.cat([h_t, ctx_t, domain_ctx_t], 1).size())
                att_t = torch.tanh(self.domain_att_vec_linear(torch.cat([h_t, ctx_t, domain_ctx_t], 1)))  # E.q. (5)
            ...
            if lstm_type == 'decoder':
                h_t, cell_t = _VF.lstm_cell(x, h_tm1, weight['decoder_lstm.weight_ih'],weight['decoder_lstm.weight_hh'], weight['decoder_lstm.bias_ih'], weight['decoder_lstm.bias_hh']) # self.decoder_lstm(x, h_tm1)
            elif lstm_type == 'entity':
                h_t, cell_t = _VF.lstm_cell(x, h_tm1, weight['entity_lstm.weight_ih'],weight['entity_lstm.weight_hh'], weight['entity_lstm.bias_ih'], weight['entity_lstm.bias_hh'])
            elif lstm_type == 'variable':
                h_t, cell_t = _VF.lstm_cell(x, h_tm1, weight['variable_lstm.weight_ih'],weight['variable_lstm.weight_hh'], weight['variable_lstm.bias_ih'], weight['variable_lstm.bias_hh'])

            ctx_t, alpha_t = self.dot_prod_attention(h_t, src_encodings, src_encodings_att_linear, mask=src_sent_masks)
            # print (src_sent_masks)
            # vector for action prediction
            att_t = torch.tanh(F.linear(torch.cat([h_t, ctx_t], 1), weight['att_vec_linear.weight'] ))  # E.q. (5)
            if self.dropout:
                att_t = self.dropout(att_t)

        return (h_t, cell_t), att_t, alpha_t

    def forward(self, examples, weight=None):
        """
        ...
        if weight is not None:
            ...
            src_encodings, (last_state, last_cell) = self.encode(src_sents_var, src_sents_len, weight)
            dec_init_vec = self.init_decoder_state(last_state, last_cell, weight)
                ...
                if self.sup_attention:
                    (action_scores, entity_scores, variable_scores), att_prob = self.decode(
                        ...
                        src_sent_masks[i].unsqueeze(0), (
                            ...
                            dec_init_vec[1][i, :].unsqueeze(0)), tgt_batch, weight)
                    if att_prob:
            ...
            if self.args.discriminate_loss:
                w_a_e = self.w_a_e.expand(len(examples), self.hidden_size*2)
                #print (src_encodings.size())
                permute_src_encodings = src_encodings.permute(1, 0, 2)
                #print (permute_src_encodings.size())
                u_c, att_weight = self.dot_prod_attention(w_a_e, permute_src_encodings, permute_src_encodings)
                #print (u_c.size())
                dis_score = self.discriminator_linear(u_c).squeeze()
                dis_p = torch.sigmoid(dis_score).clamp(min=1e-9)
                dis_loss = []
                for idx, e in enumerate(examples):
                    if e.meta == 1:
                        #print("minor loss")
                        #print (1 - dis_p[idx])
                        dis_loss.append(1 - dis_p[idx])
                    else:
                        #print("dominant loss")
                        #print(dis_p[idx])
                        dis_loss.append(dis_p[idx])
            ...
            if self.args.discriminate_loss:
                #print (stack_loss.size())
                #print (torch.stack(dis_loss).size())
                stack_loss = stack_loss + self.args.discriminate_loss*torch.stack(dis_loss).unsqueeze(1)
                #print (stack_loss)
                #print (self.args.discriminate_loss*torch.stack(dis_loss))
                #print (stack_loss)
                ret_val = [stack_loss]
            else:
                #print (stack_loss)
    ...
    def beam_search(self, example, decode_max_time_step, beam_size=5):
        ...
        if self.args.align_att:
            #print (src_encodings.size())
            #print (batch.src_sents)
            qc = self.new_tensor(1, src_encodings.size(1)).fill_(self.args.align_att)
            #print (action_seq)
            for id in example.tgt_actions[-1].align_ids:
                qc[:,id] = 1
        ...
        while len(completed_hypotheses) < beam_size and t < decode_max_time_step and len(hypotheses) > 0:
            ...
            expanded_qc = None
            if self.args.align_att:
                expanded_qc = qc.expand(hyp_num, src_encodings.size(1))
