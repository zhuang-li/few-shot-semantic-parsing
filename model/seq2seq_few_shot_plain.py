...
@Registrable.register('seq2seq_few_shot_plain')
    ...
    def decode(self, src_encodings, src_sent_masks, dec_init_vec, batch):
        ...
        for t, y_tm1_embed in list(enumerate(ast_seq_embed.split(split_size=1)))[:-1]:
                ...
                for ent in current_node.entities:
                    ...
                    if not (entity_align_id == -1):
                        ...
                        entity_att_probs.append(ratio * torch.log(
                            att_weight_e[0, entity_align_id]) + (1 - ratio) * score_e[0][self.entity_vocab.id2token[ent]])
                    """
                    e_id += 1
            ...
            if self.att_reg and (not (
                    len(action_seq[0][t + 1].prototype_tokens) == 1 and (action_seq[0][t + 1].prototype_tokens[
                                                                             0] == SLOT_PREFIX or
                                                                         action_seq[0][t + 1].prototype_tokens[
                                                                             0] == VAR_NAME or
                                                                         action_seq[0][t + 1].prototype_tokens[
                                                                             0] == 'e'))):
                # print ("sdadsada")
                        ...
                        for ent in hyp.actions[-1].entities:
                            ...
                            for src_entity_idx in entity_align:
                                ...
                                if token in self.entity_vocab.token2id:
                                    ...
                                    score_e[0][entity_vocab_id] = ratio * torch.log(att_weight_e[0][att_pos_id]) + (1 - ratio) * score_e[0][entity_vocab_id]
                            """
                            score_e[0][0] = -1000.0
                            # score_e[0][1] = -1000.0
