...
@Registrable.register('seq2seq_c_align')
    ...
    def decode(self, src_encodings, src_sent_masks, dec_init_vec, batch):
        ...
        for t, y_tm1_embed in list(enumerate(ast_seq_embed.split(split_size=1)))[:-1]:
            ...
            if self.use_coverage:
                ...
                align_cov = torch.zeros(1, len(action_seq[0]), requires_grad=False)
                if self.use_cuda:
                    align_cov = align_cov.cuda()
                ...
                src_encodings_att_linear_cov = src_encodings_att_linear.clone() + self.cov_linear(
                    torch.div(align_cov.unsqueeze(-1), torch.sigmoid(self.fertility(h_tm1[0][0]))))
            else:
