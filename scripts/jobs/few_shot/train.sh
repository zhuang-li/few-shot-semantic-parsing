...
pre_model_name=model.job.pre_train.${lstm}.hid${hidden_size}.embed${embed_size}.act${action_embed_size}.att_reg${att_reg}.drop${dropout}.dropout_i${dropout_i}.lr_decay${lr_decay}.lr_dec_aft${lr_decay_after_epoch}.beam${beam_size}.sup_turn${sup_proto_turnover}.pat${patience}.max_ep${max_epoch}.batch${batch_size}.lr${lr}.p_dropout${proto_dropout}.parser${parser}.suffix${suffix}
...
. scripts/jobs/few_shot/fine_tune.sh ${model_dir}${pre_model_name}.bin ${shuffle} ${k_shot_num} ${suffix}
#2>>logs/geo/lambda/question_split/${model_name}.log
