...
k_shot_num=$3
suffix=$4
data_dir="datasets/jobs/query_split/no_use_white_list/few_shot_split_random_3_predishuffle_${shuffle}_shot_${k_shot_num}/"
model_dir="saved_models/jobs/query_split/no_use_white_list/few_shot_split_random_3_predishuffle_${shuffle}_shot_${k_shot_num}/"
...
hidden_size=256
embed_size=200
att_reg=0.001 # 0.01 to adjust at load
action_embed_size=128
lr_decay=0.985
proto_dropout=0.5
lr_decay_after_epoch=60
max_epoch=60
...
fine_model_name=model.job.fine_tune.${lstm}.hid${hidden_size}.shuffle.${shuffle}.shot.${k_shot_num}.embed${embed_size}.drop${dropout}.dropout_i${dropout_i}.lr_decay${lr_decay}.lr_dec_aft${lr_decay_after_epoch}.beam${beam_size}.$(basename ${vocab}).$(basename ${support_file}).pat${patience}.max_ep${max_epoch}.batch${batch_size}.lr${lr}.parser${parser}.suffix${suffix}
...
. scripts/jobs/few_shot/test.sh ${model_dir}${fine_model_name}.bin ${shuffle} ${k_shot_num} ${suffix}
