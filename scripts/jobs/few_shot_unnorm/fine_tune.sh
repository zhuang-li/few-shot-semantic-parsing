...
k_shot_num=$3
suffix=$4
data_dir="datasets/jobs/query_split/unnorm/few_shot_split_random_3_predi/shuffle_${shuffle}_shot_${k_shot_num}/"
model_dir="saved_models/jobs/query_split/unnorm/few_shot_split_random_3_predi/shuffle_${shuffle}_shot_${k_shot_num}/"
...
# one shot max_epoch=80
# two shot max_epoch=100
max_epoch=80
...
fine_model_name=model.job.fine_tune.${lstm}.hid${hidden_size}.shuffle.${shuffle}.shot.${k_shot}.embed${embed_size}.drop${dropout}.dropout_i${dropout_i}.lr_decay${lr_decay}.lr_dec_aft${lr_decay_after_epoch}.beam${beam_size}.$(basename ${vocab}).$(basename ${support_file}).pat${patience}.max_ep${max_epoch}.batch${batch_size}.lr${lr}.parser${parser}.suffix${suffix}
