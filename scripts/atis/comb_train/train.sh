...
#SBATCH --output=atis-%j.out

# Set the file for error log (stderr)
#SBATCH --error=atis-%j.err
...
then
  ...
  model_dir_prefix="saved_models/atis/query_split/supervised/few_shot_split_random_5_predi/"
elif [[ $white_list -eq 1 ]]
then
  data_dir_prefix="datasets/atis/query_split/supervised/no_use_white_list/few_shot_split_random_5_predi/"
  model_dir_prefix="saved_models/atis/query_split/supervised/no_use_white_list/few_shot_split_random_5_predi/"
fi
...
. scripts/atis/comb_train/test.sh ${model_dir}${model_name}.bin ${shuffle} ${k_shot_num} ${white_list}
