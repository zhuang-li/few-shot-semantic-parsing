...
k_shot_num=$3
suffix=$4
data_dir="datasets/geo/lambda/query_split/freq_50/few_shot_split_random_5_predi/shuffle_${shuffle}_shot_${k_shot_num}/"
model_dir="saved_models/geo/lambda/query_split/freq_50/few_shot_split_random_5_predi/shuffle_${shuffle}_shot_${k_shot_num}/"
...
fine_model_name=model.geo.fine_tune.${lstm}.hid${hidden_size}.shuffle.${shuffle}.shot.${k_shot}.embed${embed_size}.drop${dropout}.dropout_i${dropout_i}.lr_decay${lr_decay}.lr_dec_aft${lr_decay_after_epoch}.beam${beam_size}.$(basename ${vocab}).$(basename ${support_file}).pat${patience}.max_ep${max_epoch}.batch${batch_size}.lr${lr}.parser${parser}.suffix${suffix}


python -u few_shot_exp.py \
    --use_cuda \
    --seed ${seed} \
    --mode fine_tune \
    --lang geo_lambda \
...
. scripts/geo/lambda/query_split/proto_train/test.sh ${model_dir}${fine_model_name}.bin ${shuffle} ${k_shot_num} ${suffix}
