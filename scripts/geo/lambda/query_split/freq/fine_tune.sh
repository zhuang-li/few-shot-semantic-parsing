...
fine_model_name=model.geo.fine_tune.${lstm}.hid${hidden_size}.shuffle.${shuffle}.shot.${k_shot_num}.embed${embed_size}.drop${dropout}.dropout_i${dropout_i}.lr_decay${lr_decay}.lr_dec_aft${lr_decay_after_epoch}.beam${beam_size}.$(basename ${vocab}).$(basename ${support_file}).pat${patience}.max_ep${max_epoch}.batch${batch_size}.lr${lr}.parser${parser}.suffix${suffix}
...
. scripts/geo/lambda/query_split/freq/test.sh ${model_dir}${fine_model_name}.bin ${shuffle} ${k_shot_num} ${suffix}
