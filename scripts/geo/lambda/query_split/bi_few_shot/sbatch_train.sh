...
#SBATCH --output=cross_template_proto-%j.out

# Set the file for error log (stderr)
#SBATCH --error=cross_template_proto-%j.err
  ...
  case $opt in
    ...
    l)
     ...
     t)
      ...
      ;;
    v)
      sup_proto_turnover=$OPTARG
      echo "supervised proto training turnover is $sup_proto_turnover"
      ;;
    \?)
...
data_dir="datasets/geo/lambda/query_split/few_shot_split_random_5_predi/shuffle_${shuffle}_shot_${k_shot}/"
model_dir="saved_models/geo/lambda/query_split/few_shot_split_random_5_predi/shuffle_${shuffle}_shot_${k_shot}/"
vocab=${data_dir}"train_vocab.bin"
train_file=${data_dir}"train.bin"

pre_model_name=model.geo.pre_train.${lstm}.hid${hidden_size}.embed${embed_size}.drop${dropout}.dropout_i${dropout_i}.lr_decay${lr_decay}.lr_dec_aft${lr_decay_after_epoch}.beam${beam_size}.$(basename ${vocab}).$(basename ${train_file}).pat${patience}.max_ep${max_epoch}.batch${batch_size}.lr${lr}.p_dropout${proto_dropout}.parser${parser}.shuffle.suffix${suffix}

python -u bi_fewshot_exp.py \
    --use_cuda \
    --seed ${seed} \
    --mode pre_train \
    --lang geo_lambda \
...
. scripts/geo/lambda/query_split/dropout_proto_few_shot/fine_tune.sh model_dir${pre_model_name}.bin
#2>>logs/geo/lambda/question_split/${model_name}.log
