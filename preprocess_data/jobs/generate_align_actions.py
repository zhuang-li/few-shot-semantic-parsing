...
def prepare_geo_align_data(train_file, support_file, query_file, dump_path_prefix, data_type='atis_lambda', lang='lambda'):
    ...
    prepare_align_actions(train_set, support_set, query_set,
                          glove_path, 300, True, map_dict=None, threshold=0.4, domain_token='job')
    """
    ...
    for s in range(6):
        for k in range(2):
            ...
            if normlize_tree == True:
                if frequency > 0:
                    dump_path_prefix = "../../datasets/jobs/query_split/freq_{}/few_shot_split_random_3_predi/shuffle_{}_shot_{}".format(frequency, shuffle, shot)
                else:
                    if use_white_list:
                        dump_path_prefix = "../../datasets/jobs/query_split/few_shot_split_random_3_predishuffle_{}_shot_{}".format(shuffle, shot)
                    else:
                        dump_path_prefix = "../../datasets/jobs/query_split/no_use_white_list/few_shot_split_random_3_predishuffle_{}_shot_{}".format(
                            shuffle, shot)
            else:
                dump_path_prefix = "../../datasets/jobs/query_split/unnorm/few_shot_split_random_3_predi/shuffle_{}_shot_{}".format(
                    shuffle, shot)
            if supvised:
                if use_white_list:
                    dump_path_prefix = "../../datasets/jobs/query_split/supervised/few_shot_split_random_3_predi/shuffle_{}_shot_{}".format(
                        shuffle, shot)
                else:
                    dump_path_prefix = "../../datasets/jobs/query_split/supervised/no_use_white_list/few_shot_split_random_3_predi/shuffle_{}_shot_{}".format(
                        shuffle, shot)
