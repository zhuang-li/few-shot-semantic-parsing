...
def prepare_geo_align_data(train_file, support_file, query_file, dump_path_prefix, data_type='geo_lambda', lang='lambda'):

    train_set, train_db = produce_data(train_file, data_type, lang,rule_type=rule_type,normlize_tree=normlize_tree,frequent=frequency, use_white_list=use_white_list)
    ...
    support_set, support_db = produce_data(support_file, data_type, lang,rule_type=rule_type,normlize_tree=normlize_tree, previous_src_list=train_src_list, previous_action_seq=train_action_list, use_white_list=use_white_list)
    ...
    prepare_align_actions(train_set, support_set, query_set,
                          glove_path, 300, True, map_dict=None, threshold=0.4,domain_token='geography')
    """

    for e in support_set:
        print (e)
        for action in e.tgt_actions:
            cond_score = torch.Tensor(action.cond_score)
            cond_score = cond_score/cond_score.sum()
            print (action)
            print ("cond_score")
            print(cond_score)

            string_sim_score = torch.Tensor(action.string_sim_score)
            string_sim_score = string_sim_score/string_sim_score.sum()

            print ("string_sim_score")
            print (string_sim_score)

    if supvised:
        train_vocab_set =train_set + support_set
                         #+ support_set
        place_holder = 6
    else:
    ...
    train_vocab = pre_few_shot_vocab(data_set=train_vocab_set,
                                     ...
                                     disjoint_set=train_set + support_set + query_set, place_holder=place_holder)
    support_vocab = pre_few_shot_vocab(data_set=train_set + support_set,
                                       entity_list=train_db.entity_list + support_db.entity_list,
                                       variable_list=train_db.variable_list + support_db.variable_list,
                                       disjoint_set=train_set + support_set + query_set)
...
if __name__ == '__main__':
    ...
    for s in range(6):
        for k in range(2):
            shuffle = s
            shot = k + 1
            path_prefix = "../../../../datasets/geo/lambda/query_split/few_shot_split_random_5_predi/shuffle_{}_shot_{}".format(shuffle, shot)
            if normlize_tree:
                if frequency > 0:
                    dump_path_prefix =  "../../../../datasets/geo/lambda/query_split/freq_{}/few_shot_split_random_5_predi/shuffle_{}_shot_{}".format(frequency, shuffle, shot)
                    print (dump_path_prefix)
                else:
                    dump_path_prefix =  "../../../../datasets/geo/lambda/query_split/few_shot_split_random_5_predi/shuffle_{}_shot_{}".format(shuffle, shot)
            else:
                dump_path_prefix = "../../../../datasets/geo/lambda/query_split/unnorm/few_shot_split_random_5_predi/shuffle_{}_shot_{}".format(
                    shuffle, shot)

            if supvised:
                if use_white_list:
                    dump_path_prefix = "../../../../datasets/geo/lambda/query_split/supervised/few_shot_split_random_5_predi/shuffle_{}_shot_{}".format(
                        shuffle, shot)
                else:
                    dump_path_prefix = "../../../../datasets/geo/lambda/query_split/supervised/no_use_white_list/few_shot_split_random_5_predi/shuffle_{}_shot_{}".format(
                        shuffle, shot)
            ...
            print (query_file)
            support_set, query_set = prepare_geo_align_data(train_file, support_file, query_file, dump_path_prefix)
            if s == 0 and k == 0:
